...
import { ArrowRight, Zap, Copy, Check, Moon, Sun, Star } from "lucide-react";
...
function Hero() {
  return (
    <section className="relative overflow-hidden bg-white dark:bg-neutral-950 pt-20 pb-28 px-4">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-gradient-to-b from-forge-100/40 to-transparent dark:from-forge-950/40 rounded-full blur-3xl pointer-events-none" />

      <div className="relative max-w-5xl mx-auto text-center">
        {/* Pill badge */}
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-forge-50 dark:bg-forge-950 border border-forge-200 dark:border-forge-800 text-forge-700 dark:text-forge-300 text-xs font-medium mb-8 animate-fade-in">
          <span className="w-1.5 h-1.5 rounded-full bg-forge-500 animate-pulse-soft" />
          240+ components · Updated weekly
          <ArrowRight size={11} />
        </div>

        <h1 className="font-display text-5xl sm:text-6xl md:text-7xl font-bold text-neutral-900 dark:text-white leading-[1.08] tracking-tight mb-6 text-balance animate-fade-up animation-delay-100">
          Ship your SaaS{' '}
        ...
        <p className="text-lg sm:text-xl text-neutral-500 dark:text-neutral-400 max-w-2xl mx-auto mb-10 leading-relaxed animate-fade-up animation-delay-200">
          A curated library of production-ready React components, SaaS sections, and full-page
          templates — built with Tailwind CSS and obsessive attention to detail.
        </p>
        ...
        <div className="flex items-center justify-center gap-6 mt-12 animate-fade-up animation-delay-400">
          <div className="flex -space-x-2">
            {['6374f1', '10b981', 'f59e0b', 'ef4444', '8b5cf6'].map((color, i) => (
              <div
                key={i}
          ...
          <div className="text-sm text-neutral-500 dark:text-neutral-400">
            <span className="font-semibold text-neutral-900 dark:text-neutral-100">2,400+</span> developers building with UIForge
          </div>
        ...
        <div className="flex h-72">
          ...
          <div className="flex-1 p-5 flex flex-col gap-4 overflow-hidden">
            <div className="flex items-center gap-3">
              ...
              <div className="h-7 w-20 bg-forge-600 rounded-lg opacity-80" />
            </div>
            <div className="grid grid-cols-3 gap-3 flex-1">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="bg-neutral-900 rounded-xl border border-neutral-800 flex flex-col overflow-hidden">
                  <div className={`flex-1 flex items-center justify-center p-3 ${i === 0 ? 'bg-neutral-800/60' : ''}`}>
                    {i === 0 ? (
                      <div className="flex gap-1.5">
                        <div className="h-6 w-14 bg-forge-600 rounded-lg" />
                      ...
                      <div className="flex gap-1">
                        <div className="px-2 py-0.5 bg-forge-900/60 text-forge-400 rounded-full text-[9px]">New</div>
                        <div className="px-2 py-0.5 bg-emerald-900/40 text-emerald-400 rounded-full text-[9px]">Active</div>
                      </div>
                    ) : (
          ...
          <div className="w-56 border-l border-neutral-800 p-4 flex flex-col gap-3 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex gap-1.5">
                <div className="px-2 py-0.5 bg-neutral-700 text-neutral-300 rounded text-[10px]">React</div>
                <div className="px-2 py-0.5 text-neutral-600 rounded text-[10px]">HTML</div>
              </div>
            ...
            <div className="flex flex-col gap-1.5">
              {[
                ...
                <div key={i} className={`h-1.5 ${line.color} rounded ${line.w}`} />
              ))}
            </div>
          </div>
        </div>
